...
TEST_F(HloConstantFoldingTest, SmallBroadcastFoldingBehavior) {
  const char* kModuleStr = R"(
    HloModule test_module
    ...
    ENTRY entry {
      %const_entry = u64[] constant(32)
      %entry_broadcast = u64[2]{0} broadcast(u64[] %const_entry), dimensions={}
      %fusion_instr = u64[2]{0} fusion(), kind=kLoop, calls=%fusion_comp
      ROOT %tuple = tuple(%entry_broadcast, %fusion_instr)
    })";
